...
Cartwright exposes 86 tools across 35 domains (the live catalog at `GET /api/v1/tools` on any shop is always the authority). Every tool can be called by an external AI agent (MCP at `/api/mcp`), a REST client (`/api/v1/tools`), the storefront chat assistant, or the admin AI — all funneled through the same `lib/tools/registry.ts`.
