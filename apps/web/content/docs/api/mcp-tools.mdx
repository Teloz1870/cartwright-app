...
Cartwright exposes <EngineFact k="toolCount" /> tools (the live catalog at `GET /api/v1/tools` on any shop is always the authority — this page is a guide, not a registry). Every tool can be called by an external AI agent (MCP at `/api/mcp`), a REST client (`/api/v1/tools`), the storefront chat assistant, or the admin AI — all funneled through the same `lib/tools/registry.ts`.
...
`orders.update_status` moves no money. It refuses `refunded` and `partial_refund` with
`refund_requires_payment_path`, because a card refund made on the admin order page or in Stripe sets
that status itself. To record a refund made outside the shop, such as a bank transfer, send
`manual: true` and `confirm: true`. A `partial_refund` also needs `refundDkk`: the amount in øre, as a positive
whole number below the order total.
The order then gets a note saying no money moved, and the result carries `moneyMoved: false`.
