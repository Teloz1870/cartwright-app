...
Cartwright is an AI-first commerce template for teams that want to own the shop instead of renting one. The private template is a Next.js 16 app with Prisma, Stripe checkout, Auth.js, admin CRUD, Vercel Blob uploads, Resend email, Sentry hooks, and an MCP server at `/api/mcp`. See [/integrations](/integrations) for every service it ships pre-wired with — fifteen of them, real brand logos, real SDK calls.
