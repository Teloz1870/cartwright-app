...
- **The engine's baseline.** Locale routing (`en` by default; add locales in `brand.config.ts`), scroll-driven motion presets with `prefers-reduced-motion` guards, security headers with a report-only CSP, an accessibility baseline, and a contact form that mails you through Resend (`--with none` for the bare site).
- **A plain Next.js repo.** `pnpm dev`, `pnpm build`, `pnpm start` — nothing else. No `postinstall`, no migrations, no seed.
...
- **Only site-level structured data.** `Organization`/`WebSite` JSON-LD ship; `Event`, `Place` or `TouristTrip` do not — add `<JsonLd>` blocks yourself if a page needs them.
...
A root `/` page and a one-page template are on the [roadmap](/docs/roadmap); this list is updated when they land.
...
The CLI ends with the truth of the matter:
...
Verify it is what it says — no database anywhere in the repo, and it boots with no environment variables:
...
- **Words.** `brand.config.ts` → `storeName`, `website.headline`, `website.tagline`, `website.cta`, `website.ctaHref`, and `url` (set it to your real domain — it feeds canonical URLs and the OG card).
- **Look.** `designSlug` in `brand.config.ts` — slugs in `designs/options.ts`. For a bespoke one-page build set `designSlug: "blank"` and write `designs/blank/homepage.tsx`; its in-file guide explains the props, and the header, footer, SEO and locale routing wrap whatever you render. Own header and footer: `designs/blank/chrome.tsx`.
...
Push to GitHub and import the repo at [vercel.com](https://vercel.com) — **no environment variables are required**. Set `RESEND_API_KEY` and `RESEND_FROM` only if the contact form should send real mail (otherwise submissions land in `.mail-previews/` in development). The step-by-step version: [From code to live](/docs/getting-started/from-code-to-live).
...
Faster to a *finished* page, not to an *empty* one. `create-next-app` finishes in seconds and hands you a blank route; `--profile site` finishes in about the same time and hands you a designed, SEO-wired site, and `next build` takes about as long in both. The time you save is the day that comes *after* `create-next-app` — layout, metadata, OG image, sitemap, locale routing, accessibility, security headers. What you do not save: a map, a timeline or a weather widget are components you write in either case. For a one-off page you will never grow, `create-next-app` is fine — and this page says so.
