...
The profile is the first of three choices — how the front gets built (a shipped pack, the blank canvas, your own pack) and where the content comes from (a URL, Shopify, WordPress/WooCommerce, a CSV) are the other two, and the origin can force the profile: [Choose your path](/docs/getting-started/choose-your-path).
...
- **A plain Next.js repo.** `pnpm dev`, `pnpm build`, `pnpm start`. No `postinstall`, no migrations, no seed. Sentry's hooks ship dormant (no DSN, no-op) — nothing calls out at runtime unless you give it a key. Three of the <EngineFact k="siteRuntimeDeps" /> runtime dependencies are dormant — the MCP SDK, `jsdom` and `v0-sdk` are installed, but nothing in this profile's runtime imports them.
