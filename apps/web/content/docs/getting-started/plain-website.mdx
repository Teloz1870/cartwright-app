...
- **A plain Next.js repo.** `pnpm dev`, `pnpm build`, `pnpm start`. No `postinstall`, no migrations, no seed. Sentry's hooks ship dormant (no DSN, no-op) — nothing calls out unless you give it a key.
...
- **Words.** `brand.config.ts` → `storeName`, `website.headline`, `website.tagline`, `website.cta`, `website.ctaHref`, and `url` — your real domain. On Vercel, `NEXT_PUBLIC_APP_URL` (or the deployment's own URL) wins over it for canonical URLs; `url` feeds the site's JSON-LD and the OG card.
