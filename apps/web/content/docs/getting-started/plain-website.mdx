...
- **A plain Next.js repo.** `pnpm dev`, `pnpm build`, `pnpm start`. No `postinstall`, no migrations, no seed. Sentry's hooks ship dormant (no DSN, no-op) — nothing calls out at runtime unless you give it a key.
