...
6. **Deploy.** Push to GitHub and import the repo in Vercel, or run `npx vercel` from the terminal — **no environment variables are required to build and serve** (the default contact form needs `RESEND_API_KEY` + `RESEND_FROM` to deliver mail; `--with none` scaffolds without it). Set `url` in `brand.config.ts` to the real domain when you have one; on Vercel, `NEXT_PUBLIC_APP_URL` or the deployment URL wins over it for canonicals.

   ```bash
   gh repo create my-site --private --source=. --remote=origin --push
   ```

   **Verify** against the URL Vercel prints: `curl -sI <that-url>/ | grep -i location` → `/en`, and `/en` → `200`.
