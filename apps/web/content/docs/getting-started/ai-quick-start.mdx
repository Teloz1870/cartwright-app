...
The current CLI ships `light`, `full`, and database-free `site` profiles. This runbook uses `light` (the default) because its database, admin, and scoped tool APIs are required below; `site` intentionally has none of those surfaces.
</Callout>
