...
| --- | --- | --- |
| a page, a landing page, a personal or company website with content in the repo — no database, no login | `npx create-cartwright@latest my-site --yes --profile site` | **Path A** below |
| a site someone edits in a browser, a webshop, or a site an agent should operate through tools | `npx create-cartwright@latest my-site` | **Path B** below |
...
Measured cold run (<EngineFact k="siteColdRunProvenance" />): scaffold + install <EngineFact k="siteColdRunScaffold" /> · `next build` <EngineFact k="siteColdRunBuild" /> · boot to HTTP 200 with a rendered H1 <EngineFact k="siteColdRunBoot" />.
...
1. **Scaffold — no database, no login, nothing to configure.**
   ...
   ```bash
   npx create-cartwright@latest my-page --yes --profile site --no-github --no-start
   cd my-page
   ```
...
2. **Run.**
   ...
   *Proves:* it boots with zero environment variables.
...
4. **Add a page** — `app/[locale]/trip/page.tsx` is a normal Next.js route; the `[locale]` layout gives it the site chrome. Prefix internal links with `/${locale}`.
...
5. **Build.**
   ...
   *Proves:* plain `next build` is the whole production gate — no `db:setup`, no migration.

6. **Deploy.** Push to GitHub and import the repo in Vercel — **no environment variables**. Set `url` in `brand.config.ts` to the real domain when you have one (it feeds canonical URLs and the OG card).

   ```bash
   gh repo create my-page --private --source=. --remote=origin --push
   ```

   **Verify:** `curl -sI https://<app>.vercel.app/ | grep -i location` → `/en`, and `/en` → `200`.

</Steps>

Honest limits of this door: no admin and no runtime editing (content is files), no database/auth/cart/checkout, no MCP or REST tool surface on the site itself, `/` redirects to the default locale, no map/timeline/weather sections (write those components yourself), only `Organization`/`WebSite` JSON-LD out of the box, not a static export (Node.js 22+ host). Need a browser editor, a shop or an agent tool surface later? There is no in-place profile upgrade today — scaffold the default profile and copy `brand.config.ts` and `designs/<yours>/` across. Full page: [Build a plain website](/docs/getting-started/plain-website).
