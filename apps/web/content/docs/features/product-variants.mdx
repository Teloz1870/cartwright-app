...
The CSV flow is also the bulk path for simple products out of a Shopify or WooCommerce export — the ten columns are fixed (`slug, name, description, priceKr, stock, brand, featured, categorySlug, images, attributes`), categories must exist first, and variants, SKU and weight are not imported yet.
