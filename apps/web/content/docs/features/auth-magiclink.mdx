...
<Callout type="info">
On a **brand-new shop** there's no `RESEND_API_KEY` yet, so the login page hides magic-link and shows only the password tab. For your very first admin login, use the seeded password — see [Sign in for the first time](/docs/getting-started/first-login).
</Callout>
