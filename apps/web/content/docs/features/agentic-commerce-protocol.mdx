...
- `POST /api/acp/v1/checkout_sessions/{id}/complete` — finalise the session. Gated behind `ACP_PAYMENT_COMPLETION=1` (default off): the gate and the `ready_for_payment` status validation are live, but the delegated-payment charge is a structured scaffold (see the note below) until Stripe Shared Payment Tokens are wired — so it can never accidentally move money
...
When fully wired, `complete` returns the order id and a status — `succeeded`, `requires_action`, or `failed`; ACP buyer agents look for `requires_action` to surface a 3DS or SCA challenge URL back to the originating chat thread.

<Callout type="warn">
**Completion is a default-off scaffold today.** `/complete` ships behind `ACP_PAYMENT_COMPLETION=1` (default off). The verifiable parts — the env gate and the `ready_for_payment` status validation — are real; the one external step (charging via a shared payment token) throws `payment_not_wired` until Stripe Shared Payment Token access and ChatGPT merchant onboarding are connected. The endpoint therefore cannot charge a card while the integration is still being finished. `create` / `update` / `retrieve` / `cancel` are fully live.
</Callout>
