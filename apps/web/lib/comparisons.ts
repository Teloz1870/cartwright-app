...
const woocommerce: Comparison = {
  ...
  faq: [
    { q: 'Can I migrate from WooCommerce to Cartwright?', a: 'Partly, today — and this page says so. Products move by CSV import (simple products; variants, SKU and weight not yet), pages and posts by the URL import as drafts, and old URLs by admin-managed redirects (exact paths, one at a time, Upstash Redis required; `/?p=123` query permalinks are not matched yet). A dedicated WordPress/WooCommerce importer is planned, not built; the Hoptify importer is for Shopify and needs `--profile full`. Orders and customers never move. What you get is a modern TypeScript codebase you own — see Choose your path (cartwright.app/docs/getting-started/choose-your-path).' },
...
const createNextApp: Comparison = {
  ...
  answer:
    `Both are one-command Next.js scaffolds — the difference is what exists when the command finishes. create-next-app hands you a blank canvas: routing and tooling, no pages, no data layer, no admin. create-cartwright hands you a running product on the same stack. Two doors: ${INSTALL_COMMAND_SITE} is a plain website — designed pages, JSON-LD, sitemap, llms.txt, an OG image route, locale routing, a contact form; no database, no login, ${ENGINE_FACTS.siteRuntimeDeps} runtime dependencies, plain next build. The default profile adds a Prisma database with seeded content, an admin with auth, optional cart/checkout and an AI tool surface your coding agent can drive. Start blank when the app is genuinely novel and you want zero opinions; start with Cartwright when the goal is a website or shop — you skip the first day (or two weeks) of plumbing and still own every line, because it is your repo either way.`,
  rows: [
    { dimension: 'What exists after the command', cartwright: 'A running site: design system, SEO/JSON-LD, locale routing — and, in the default profile, a seeded database, admin, auth and a webshop if you want it.', them: 'A blank app directory with routing, TypeScript and Tailwind configured. Everything else is yours to build.' },
    { dimension: 'A single page or plain website', cartwright: `--profile site: a designed, SEO-wired site with no database or login — ${SITE_COLD_RUN.scaffold} to scaffold and install, ${SITE_COLD_RUN.build} to build, ${SITE_COLD_RUN.boot} to a rendered homepage (measured). A map, a timeline or a weather widget are components you write in either case.`, them: 'A blank route in seconds; layout, metadata, OG image, sitemap, locale routing, accessibility and security headers are the day that comes after.' },
  ...
  faq: [
    ...
    { q: 'When is create-next-app the better choice?', a: 'When you are building something that is not a website — a dashboard, an internal tool, a novel app — and you want zero opinions in the repo; blank is a feature there. And for a one-off page that needs neither design nor discovery, create-next-app is as fast to an empty route.' },
    { q: 'Is it faster than create-next-app for a single page?', a: `Faster to a finished page, not to an empty one. create-next-app finishes in seconds and hands you a blank route; ${INSTALL_COMMAND_SITE} finishes in ${SITE_COLD_RUN.scaffold} and hands you a designed, SEO-wired site that builds in ${SITE_COLD_RUN.build} — the same next build command in both. The time you save is the day that comes after create-next-app — layout, metadata, OG image, sitemap, locale routing, accessibility, security headers. What you do not save: a map, a timeline or a weather widget are components you write in either case.` },
