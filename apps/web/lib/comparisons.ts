...
const woocommerce: Comparison = {
  ...
  faq: [
    { q: 'Can I migrate from WooCommerce to Cartwright?', a: 'Partly, today — and this page says so. Products move by CSV import (simple products; variants, SKU and weight not yet), pages and posts by the URL import as drafts, and old URLs by admin-managed redirects (exact paths, one at a time; `/?p=123` query permalinks are not matched yet). A dedicated WordPress/WooCommerce importer is planned, not built; the Hoptify importer is for Shopify and needs `--profile full`. Orders and customers never move. What you get is a modern TypeScript codebase you own — see Choose your path (cartwright.app/docs/getting-started/choose-your-path).' },
