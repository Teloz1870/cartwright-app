...
const createNextApp: Comparison = {
  ...
  faq: [
    ...
    { q: 'When is create-next-app the better choice?', a: 'When you are building something that is not a website — a dashboard, an internal tool, a novel app — and you want zero opinions in the repo; blank is a feature there. And for a one-off page you will never grow, create-next-app is as fast — say so.' },
    { q: 'Is it faster than create-next-app for a single page?', a: `Faster to a finished page, not to an empty one. create-next-app finishes in seconds and hands you a blank route; \`${INSTALL_COMMAND_SITE}\` finishes in ${SITE_COLD_RUN.scaffold} and hands you a designed, SEO-wired site, and next build takes ${SITE_COLD_RUN.build} either way. The time you save is the day that comes after create-next-app — layout, metadata, OG image, sitemap, locale routing, accessibility, security headers. What you do not save: a map, a timeline or a weather widget are components you write in either case.` },
