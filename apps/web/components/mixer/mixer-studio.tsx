...
// The public Mixer studio — a Skin × Voice composer over the vendored
// marketplace manifest (static-mock preview).
//
// TODO(mixer-live-preview): the composed preview below is a manifest-rendered
// mock (voice copy + palette + motif), not the real engine page. The planned
// upgrade is a live iframe against an engine mixer-preview deployment — the
// engine already ships the gated /<locale>/mixer-preview route and the admin
// Page Mixer drives exactly that. Standing up a public preview deploy is an
// owner decision (infra + cost), so the static mock ships first.
//
...
import type { MixerDesignEntry, Palette, VoiceEntry } from '@/lib/marketplace';
...
export function MixerStudio({
  ...
  // Deep-linking: /mixer?skin=<slug>&voice=<slug> (e.g. "Open in the Mixer"
  // from a Look card). Read once on mount — window-based so the page itself
  // stays fully static.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    window.history.replaceState(null, '', `/mixer?skin=${skinSlug}&voice=${voiceSlug}`);
  }, [hydratedFromUrl, skinSlug, voiceSlug]);
  ...
  const agentPrompt = `Apply the "${voice.name}" Voice and the "${design.name}" design to my Cartwright store. Use the magic.compose_look tool with { vertical: "${voice.slug}", design: "${design.slug}" } — one audited step that re-tones the copy, sets the palette + 3D scene, and switches the active design.`;
        ...
        <div className="overflow-hidden rounded-2xl border border-cw-stone-200 bg-white shadow-sm dark:border-cw-stone-700 dark:bg-cw-stone-900">
          ...
          <div className="flex flex-col gap-3 p-5">
            ...
            <p className="text-xs leading-relaxed text-cw-stone-500 dark:text-cw-stone-400">
              ...
              real sample copy in the Skin&rsquo;s default palette. Palette-adaptive Skins re-colour
              to your brand / Voice palette when applied; a live full-page preview is the planned
              upgrade.
            </p>
          </div>
        </div>
