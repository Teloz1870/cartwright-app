...
/**
 * Voice-plan Fase 3.1: forside-demo — embedded mini-storefront.
 *
 ...
 *
 * Component er self-contained: WS-wrapper + AudioContext + UI i én fil for
 * marketing-side simplicity. Test-templatens VoiceShopClient/Overlay er den
 * "rigtige" full-feature implementation; her er det "1 min wow-demo".
 */
...
export function VoiceShopDemo() {
  ...
  useEffect(() => {
    if (secondsLeft === 0 && state !== "ended" && state !== "error") {
      stopDemo();
    }
  }, [secondsLeft, state]);
  ...
  return (
    <section className="relative overflow-hidden border-y border-cw-stone-200 bg-cw-stone-50 dark:border-cw-stone-800 dark:bg-cw-stone-950">
      <div className="mx-auto max-w-4xl px-6 py-16 sm:py-20">
        <div className="flex flex-col items-center text-center">
          <span className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-cw-terracotta/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-cw-terracotta">
            <span className="size-1.5 rounded-full bg-cw-terracotta" /> live
            demo
          </span>
          <h2 className="max-w-2xl text-3xl font-semibold tracking-tight text-cw-stone-900 dark:text-cw-stone-50 sm:text-4xl">
            Try voice + vision shopping right now
          </h2>
          <p className="mt-3 max-w-xl text-sm text-cw-stone-600 dark:text-cw-stone-300">
            Click the mic. Ask for &ldquo;coffee&rdquo; or &ldquo;a reading
            lamp&rdquo;. Hear the demo store talk back. 30 seconds, no signup.
          </p>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            {!active && state !== "ended" && (
              <button
                ...
                className="group flex h-20 w-20 items-center justify-center rounded-full bg-cw-terracotta text-white shadow-lg shadow-cw-terracotta/30 transition hover:scale-105 hover:bg-cw-terracotta/90 disabled:opacity-50"
            ...
            {error && (
              <p className="rounded-md border border-cw-stone-300 bg-white px-3 py-2 text-sm text-cw-stone-700 dark:bg-cw-stone-900 dark:text-cw-stone-200">
                {error}
              </p>
            )}
          </div>
