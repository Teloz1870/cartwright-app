...
- be120d2: `--help` no longer implies a webshop needs `--profile full`: the profile and template notes now say a webshop is `--profile light --template generic`, and that `full` only adds the A2A agent marketplace, UCP identity-linking, the Shopify (Hoptify) importer and the uncurated design packs (MCP, ACP and WebMCP ship in light too). The stale "all 26 designs" count is gone from the help text (the engine ships 30; a number in `--help` drifts). Measured 2026-09-06: three of three AI replays moving a WooCommerce shop scaffolded `--profile full` after reading the old text.
- 3cb67ae: Stop scaffolds from shipping Cartwright's own identity, and gate the anchors so it cannot silently happen again.
  ...
  - **`company.sameAs`** kept Cartwright's GitHub + npm profiles. `app/layout.tsx` feeds this into Organization JSON-LD, so every scaffolded site published machine-readable structured data asserting the customer's company _is_ the Cartwright project.
...
- c491939: A scaffold no longer ships our identity or scripts it cannot run.
...
- a7275bf: Bump default template ref to v0.56.2 (was v0.56.1).

## 2.9.3

### Patch Changes

- 5212d84: Bump default template ref to v0.56.1 (was v0.55.0).
...
- 82b5e06: Bump default template ref to v0.51.0 (was v0.50.1).

## 2.8.0

### Minor Changes

- 1536e79: Webshop templates (`--template coffee | sunglasses | generic`) now scaffold with `brand.features.webMcp: true` — the store is WebMCP-native out of the box: page-contextual browser tools register the moment an agent-capable browser opens it, while the render stays byte-identical everywhere else. Website-mode and agent-marketplace scaffolds keep the flag off (it carries an ecommerce precondition). README/help/docs updated to the same story.

## 2.7.15

### Patch Changes

- 2a3ffcf: Bump default template ref to v0.50.1 (was v0.50.0).

## 2.7.14

### Patch Changes

- 0eacf86: The light (default) profile keeps the full WebMCP surface — and stops shipping a broken scaffold.
...
- 619c5db: Bump default template ref to v0.50.0 (was v0.49.0).

## 2.7.12

### Patch Changes

- 6f8acdf: Light profile prunes the WebMCP 2.0 unit-test surface (five upcoming engine
...
- 1548e5a: Bump default template ref to v0.49.0 (was v0.48.0).
...
- ae29d5a: Bump default template ref to v0.47.0 (was v0.46.0).
...
- 8b805cf: Bump default template ref to v0.45.0 (was v0.44.1).

## 2.7.5

### Patch Changes

- 14f1008: Stop patching `/icon` into the proxy matcher on engines that exempt it themselves.
...
- d25d57b: Bump default template ref to v0.44.1 (was v0.44.0).

## 2.7.3

### Patch Changes

- dff23a1: Bump default template ref to v0.44.0 (was v0.43.0).

## 2.7.2

### Patch Changes

- 4a1b9e0: Bump default template ref to v0.43.0 (was v0.42.0).

## 2.7.1

### Patch Changes

- d98c482: Bump default template ref to v0.42.0 (was v0.41.0).
...
- 4d17dbb: Bump default template ref to v0.41.0 (was v0.40.0).
...
- b26f243: Light profile: prune `tests/unit/modules.test.ts` as well — its "every declared module file exists on disk" invariant pins the full module graph (light prunes design packs). The release scaffold-gate's boot step also sets `ALLOW_SQLITE_IN_PRODUCTION=1` so the engine's production sqlite guard (launch-hardening) doesn't fail the sqlite-based gate legs.

## 2.6.1

### Patch Changes

- f97f0de: Light profile: prune the engine's B3 module-manifest tests (scaffold-manifest drift, site-profile import closure) and the newer `ucp-capability-profile` test — they pin the FULL module graph/registry and fail typecheck or assertions in a pruned light scaffold (first surfaced by the ref=next scaffold-gate run).
...
- 2854477: npm README overhaul: correct quickstart (the CLI already installs, creates the schema and
  seeds — the published README wrongly instructed `prisma migrate deploy` + `db seed`),
  badges, live-demo links, and richer npm keywords (shopify-alternative, agentic-commerce,
  mcp-server, create-next-app, storefront).

## 2.5.1

### Patch Changes

- e494aca: Bump default template ref to v0.39.1 (was v0.39.0).

## 2.5.0

### Minor Changes

- 26cea03: New `--look <url>` flag — scaffold wearing a shared look (`cartwright-composition-v1`).
...
- 59cf29e: Bump pinned `modern-web-guidance` version to 0.0.174 (was 0.0.170).
- 4c8bcf2: Bump default template ref to v0.39.0 (was v0.38.0).
...
- 22eb8e9: Bump default template ref to v0.37.1 (was v0.37.0).
...
- 765b527: npm front door fixed: the published package now carries its README on npmjs.com (pnpm `embedReadme` — `pnpm publish` defaults it off, so every previous version shipped an empty registry readme despite README.md being inside the tarball), the package description now tells the real product story, and the README no longer claims the template repo is private (it is public + MIT).

## 2.3.4

### Patch Changes

- da8fde2: Prefer pnpm when present: `npx create-cartwright` always reports npm in the user agent, but every doc in the scaffold says `pnpm <cmd>` — an npm-locked scaffold guaranteed a package-manager mismatch on the first `pnpm add`. Explicit `--pm` still wins. Also: the light profile no longer warns about deps already removed at the engine source.
- 8e45b8a: Bump default template ref to v0.36.3 (was v0.36.2).

## 2.3.3

### Patch Changes

- 25d3642: Bump default template ref to v0.36.2 (was v0.36.1).
...
- b90a0ca: Bump default template ref to v0.34.0 (was v0.33.0).

## 2.1.2

### Patch Changes

- cb6a510: Fix `cartwright vertical install`: its `DEFAULT_REF` was pinned to v0.32.0 (before the verticals registry existed), so `stable` resolved to a tag with no `verticals/` directory and the install failed. Bumped to v0.33.0 and wired into the auto-bump workflow so it tracks future releases alongside the scaffolder and design-install.

## 2.1.1

### Patch Changes

- f115a49: Bump default template ref to v0.33.0 (was v0.32.0).

## 2.1.0

### Minor Changes

- bbb0fae: Add `cartwright design install <slug>` — install a marketplace design into an existing Cartwright project.
...
- d49f277: Bump default template ref to v0.32.0 (was v0.31.0).

## 2.0.48

### Patch Changes

- b8a7d13: Bump default template ref to v0.31.0 (was v0.30.0).

## 2.0.47

### Patch Changes

- 00249f2: Bump default template ref to v0.30.0 (was v0.29.0).

## 2.0.46

### Patch Changes

- cd857a4: Bump default template ref to v0.29.0 (was v0.28.0).

## 2.0.45

### Patch Changes

- 732ca6b: Bump default template ref to v0.28.0 (was v0.27.1).

## 2.0.44

### Patch Changes

- ce839a7: Bump default template ref to v0.27.1 (was v0.27.0).

## 2.0.43

### Patch Changes

- 22ff22a: Bump default template ref to v0.27.0 (was v0.26.0).

## 2.0.42

### Patch Changes

- 917b5cc: Scaffold first-run now prefers the engine's robust `db:setup` (v0.27.0+): it tries `prisma db push` and, if the intermittent Prisma 7.8 schema-engine error hits, falls back to applying the schema via the libSQL client (bypassing the flaky schema engine) before seeding — so onboarding can't get stuck. Older templates without `db:setup` keep the legacy push (+retry) + seed path. The "Next steps" list and the failure message now point at `db:setup` instead of telling the user to "just run it again."

## 2.0.41

### Patch Changes

- beaaa8e: Bump default template ref to v0.26.0 (was v0.25.0).

## 2.0.40

### Patch Changes

- e7a2e5f: Bump default template ref to v0.25.0 (was v0.24.1).

## 2.0.39

### Patch Changes

- 7625e0a: Bump default template ref to v0.24.1 (was v0.24.0).

## 2.0.38

### Patch Changes

- 4a491c1: Bump default template ref to v0.24.0 (was v0.23.0).

## 2.0.37

### Patch Changes

- f088e34: Bump default template ref to v0.23.0 (was v0.21.0).

## 2.0.36

### Patch Changes

- 071bf39: Bump default template ref to v0.21.0 (was v0.19.0).

## 2.0.35

### Patch Changes

- 3510b05: Bump default template ref to v0.19.0 (was v0.18.0).
