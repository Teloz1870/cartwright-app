...
One behaviour does change: giget 1 read `HTTPS_PROXY`/`HTTP_PROXY` itself, giget 3 uses Node's own `fetch`, which honours them only when the process runs with `NODE_USE_ENV_PROXY=1` (Node 24+). When a template fetch fails and a proxy variable is set, the CLI now says exactly that.
